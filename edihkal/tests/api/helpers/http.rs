/// This helper provides a wrapper of the reqwest client for integration testing.
///
/// `TestClient` wraps the client's HTTP methods to allow relative URL paths built
/// using the address it's constructed with.
///
/// This is a modified version of `axum-test-helper` with the service initialization
/// removed (handled elsewhere in `helpers`).
use axum::http::{
    self,
